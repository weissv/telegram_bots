...
export function Header({
  ...
  const toggleLocale = () => {
    onLocaleChange(locale === 'ru' ? 'uz' : 'ru');
  };
  ...
  return (
    <header className="sticky top-0 z-30 bg-slate-900/90 backdrop-blur-md border-b border-slate-800/80 px-4 py-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-750 text-slate-300 border border-slate-700/60 text-xs font-semibold transition-all active:scale-95"
      ...
      <div className="mt-3 relative">
        ...
        <input
          type="text"
          placeholder={t('miniapp.search_placeholder')}
          value={searchQuery}
